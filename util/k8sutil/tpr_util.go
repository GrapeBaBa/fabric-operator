...
func WatchChains(host, ns string, httpClient *http.Client, resourceVersion string) (*http.Response, error) {
...
func GetChainList(restcli rest.Interface, ns string) (*spec.ChainList, error) {
	b, err := restcli.Get().RequestURI(listChainsURI(ns)).DoRaw()
	if err != nil {
		return nil, err
	}

	clusters := &spec.ChainList{}
...
func WaitChainTPRReady(restcli rest.Interface, interval, timeout time.Duration, ns string) error {
	return retryutil.Retry(interval, int(timeout/interval), func() (bool, error) {
		_, err := restcli.Get().RequestURI(listChainsURI(ns)).DoRaw()
		if err != nil {
...
func listChainsURI(ns string) string {
