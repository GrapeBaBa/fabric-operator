...
type OrdererServiceListCopy OrdererServiceList
type OrdererServiceCopy OrdererService

func (c *OrdererService) UnmarshalJSON(data []byte) error {
	tmp := OrdererServiceCopy{}
	err := json.Unmarshal(data, &tmp)
	if err != nil {
		return err
	}
	tmp2 := OrdererService(tmp)
	*c = tmp2
	return nil
}

func (cl *OrdererServiceList) UnmarshalJSON(data []byte) error {
	tmp := OrdererServiceListCopy{}
	err := json.Unmarshal(data, &tmp)
	if err != nil {
		return err
	}
	tmp2 := OrdererServiceList(tmp)
	*cl = tmp2
	return nil
}
